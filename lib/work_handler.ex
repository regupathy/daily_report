defmodule WorkHandler do
  ...
  defstruct [:id,:job_name,:start_row,:destination,:sql_worker, :work,:reporter, current_row: 0,
   items_count: 5, cache: [], cache_count: 0]

  def params(id,job_name,row,destination,items_count \\ 5) do
    %WorkHandler{id: id,job_name: job_name,destination: destination, start_row: row,items_count: items_count}
  end

  def init({%WorkHandler{job_name: job_name} = state,reporter}) do
    work = Work.get(job_name)
    case File.exists?(Work.get_source(work)) do
      true -> GenServer.cast(self(),{:start,self()})
        {:ok,%WorkHandler{state | work: work, reporter:  reporter}}
      false -> {:stop,:file_not_found}
    end
  end
  ...
  def handle_cast({:start,pid},%WorkHandler{work: work,destination: destination,start_row: start_row} =state) when pid == self() do
  ...
  def handle_cast({:row,number,field_values}, %{item_count: n ,cache_count: m} = state) when m+1 == n do
  ...
  def handle_cast({:row,number,field_values}, %{cache_count: m, cache: caches  } = state) do
    {:noreply, %{state| cache: caches ++ [field_values], current_row: number, cache_count: m+1 }}
  end
